...
// 				});
        	 ...
        	 $.get("act_Authority", function(responseJson) {          // Execute Ajax GET request on URL of "someservlet" and execute the following function with Ajax response JSON...
 		        ...
 		        $.each(responseJson, function(index, product) {    // Iterate over the JSON array.
 		            $("<tr>").appendTo($table).addClass('article-loop')                     // Create HTML <tr> element, set its text content with currently iterated item and append it to the <table>.
 		                .append($("<td>").text(product.authorityID).css('width','350px'))        // Create HTML <td> element, set its text content with id of currently iterated product and append it to the <tr>.
 		                .append($("<td>").text(product.authorityName))      // Create HTML <td> element, set its text content with name of currently iterated product and append it to the <tr>.
 		                .append($("<td>")
 		                		.append($("<a>")
 		                				.append($("<i>").addClass("fa fa-pencil edit"))
 		                			   )
 		                				.click(function(){
 		                		           	 $.post("act_Authority",{action: '3',authorityID : product.authorityID}, 
 		                		           		function(responseJson) { 
	 		                		           		if (responseJson.check == "fail") {
		 		               		                  	alert("Loaddata failed!");
		 		               		                    return;
	 		               		       				 }
 		                		     		        	$("#contentGrID2").val(responseJson.authorityID);
 		                		     		        	$("#contentGrName2").val(responseJson.authorityName);
 		                		     		    }
 		                		           	 );
 		                		...
 		                		.append($("<a>")
 		                				...
 		                				.click(function(){
 		                					var retVal = confirm("Do you really want to delete this?");          	
	 		           	                    if( retVal == true ){
	 		           	                    	$.post("act_Authority",{action: '4',authorityID : product.authorityID}, 
	   		                						function(responseJson) {  
		   		                						if (responseJson.check == "fail") {
		                ...
		                if (listSize > 0) {
		                   ...
		                   var id2=null;
		                    for (i = 0; i < listSize; i++) {
		                        ...
		                        cell.innerHTML = result[i].authorityID;
		                        cell1.innerHTML = result[i].authorityName;
		              		    cell2.innerHTML = "<a><i></i></a>";
		              		    cell3.innerHTML = "<a><i></i></a>";

		                        // Thêm class vào tr
		                        row.className += 'text-center';// class có tên text-center
		                        // Thêm class vào td
		                        cell.className += 'text-center'; // Class có tên text-center
		                        // Thêm class vào td tạo kích thước width cho từng cột
		                        cell1.className += 'text-center';
		                        cell2.className+='fa fa-pencil edit text-center';
                             	cell3.className+='fa fa-trash remove text-center';
                             	cell2.click(function(){
	                					id=product.authorityID;
		                					alert(id);
		                				})
        	...
        	*/
        	...
        	$('#add1').click(function(){
           	 $.post("act_Authority",{action: '1',authorityName : $('#contentGrName').val()}, function(responseJson) {          // Execute Ajax GET request on URL of "someservlet" and execute the following function with Ajax response JSON...
           		if (responseJson.check == "fail") {
                ...
                $.ajax({
                	...
                	success: function(result){
		                ...
		                if (result.check == "fail") {
		                    //$('#message').text("List isEmpty! Name not found!");
		                    //$('#message').css('color', 'red');
		                  	alert("Load Authority DB, Fail!");
		                    return;
		                }
		                else if(result.check == "fail2"){
		                	alert("Add new authority, Fail!");
		                	return;
		                }
		                ...
		                if (listSize > 0) {
		                    ...
		                    for (i = 0; i < listSize; i++) {
		                        ...
		                        if(row==null)
		                        {
			                        ...
			                        cell.innerHTML = result[i].authorityID;
			                        cell1.innerHTML = result[i].authorityName;
			              		    cell2.innerHTML = result[i].html="<a><i></i></a>";
			              		    cell3.innerHTML = result[i].html="<a><i></i></a>";
        	...
        	$('#edit1').click(function(){
           	 $.post("act_Authority",{action: '2',authorityID : $('#contentGrID2').val(),authorityName : $('#contentGrName2').val()},
        ...
        <div class="md-content">
            ...
            <form id="login_form_contentGr" class="was-validated" action="" method="">
                ...
                <!--  
                        ...
                        -->
                        <div>
                            <label for="text">Authority Name</label>
                            <span class="red_right_add2e" id="contentGrName_error_message"></span>
                            <!-- cột hiện lỗi validate -->
                        </div>
                        <div>
                            <input type="text" name="authorityName" id="contentGrName" placeholder="Enter Authority Name">
                        </div>
                </fieldset>
                <div>
                    <div id="btn">
                        ...
                        <input class="animated" type="submit" value="ADD">
                           ...
                           <input id="closebtn_form2" data-dismiss="modal-4options" aria-label="Close"
        ...
        <div class="md-content">
            ...
            <form id="login_form_contentGr2" class="was-validated" action="" method="post">
                <fieldset>
                        ...
                        <div>
                            <input type="text" name="authorityID" id="contentGrID2" placeholder="Enter Authority ID" 
                              disabled>
                        </div>
                        <div>
                            <label for="text">Authority Name</label>
                        ...
                        <div>
                            <input type="text" name="authorityName" id="contentGrName2" placeholder="Enter Authority Name">
                        </div>
                </fieldset>
                <div>
                    <div id="btn">
    ...
    <div class="md-modal5 md-effect-7" id="modal-5options">
        <div class="md-content">
            <h4 class="h4-repair-contentGr">
                <span>
                        Delete Authority
                        <button type="button" class="close" data-dismiss="modal-5options" aria-label="Close" id="closebtn4">
                              <span aria-hidden="true">&times;</span>
                        </button>
                </span>
            </h4>
            ...
            <form id="login_form_5options" class="was-validated" action="" method="post">
                <fieldset>
                        <div>
                            <label for="text">Authority ID</label>
                            <span class="red_right_add2e" id="contentGrID_error_message" style="margin-right: 40px;"></span>
                        </div>
                        <div>
                            <input type="text" id="contentGrID" placeholder="Enter Authority ID">
                        </div>
                </fieldset>
                <div>
                    <div id="btn">
                        <input class="animated" type="submit" value="DELETE">
                        <input class="animated md-close" type="submit" value="CANCEL">
                    </div>
                </div>
            </form>
        </div>
 ...
 <style>
    ...
    <!-- Authority Manegement -->
     ...
     <div class="container tieude" id="content-gr">
         		<h2 class="text-center" style="color: red;font-weight: bold;margin-bottom: 20px !important;">AUTHORITY MANEGEMENT</h2>
        <p class="text-right">
        <a class="md-trigger btn" data-modal="modal-3options" id="btn_left"><button type="submit" class="btn add-employee"><i class="fa fa-plus" aria-hidden="true"></i>Add Authority</button></a>
        </p>
