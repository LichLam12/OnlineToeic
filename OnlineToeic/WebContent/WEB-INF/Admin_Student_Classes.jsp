...
// 				});
        	...
        	$.post("act_StudentClass",{action: '6'},function(responseJson) {
     			 ...
     			 $.each(responseJson, function(index, product) {    // Iterate over the JSON array.
   		        	$("#classID").append($("<option>").val(product.classID)
	     		       				.text(product.className))
	     		    $("#classID2").append($("<option>").val(product.classID)
	     		       				.text(product.className))
	     		       	
     		 	});
     		 });
        	
        	$.post("act_StudentClass",{action: '5'},function(responseJson) {
   			 if (responseJson.check == "fail") {
  		        	alert("Loaddata failed!");
		                return;       		                  	
	 		        }
   			 ...
   			 $.each(responseJson, function(index, product) {    // Iterate over the JSON array.
 		        	$("#studentID").append($("<option>").val(product.studentID)
        	 ...
        	 $.get("act_StudentClass", function(responseJson) {          // Execute Ajax GET request on URL of "someservlet" and execute the following function with Ajax response JSON...
 		        ...
 		        $.each(responseJson, function(index, product) {    // Iterate over the JSON array.
 		            $("<tr>").appendTo($table).addClass('article-loop')                     // Create HTML <tr> element, set its text content with currently iterated item and append it to the <table>.
 		                .append($("<td>").text(product.studentclass_ID))        // Create HTML <td> element, set its text content with id of currently iterated product and append it to the <tr>.
 		                .append($("<td>").text(product.studentID))      // Create HTML <td> element, set its text content with name of currently iterated product and append it to the <tr>.
 		               	.append($("<td>").text(product.classID))
 		                ...
 		                .append($("<td>")
 		                		.append($("<a>")
 		                				...
 		                				.click(function(){
 		                		           	 $.post("act_StudentClass",{action: '3',studentclass_ID : product.studentclass_ID}, 
 		                		           		function(responseJson) { 
	 		                		           		if (responseJson.check == "fail") {
		 		               		                  	alert("Loaddata failed!");
		 		               		                    return;
	 		               		       				 }
 		                		     		        	$("#contentGrID2").val(responseJson.studentclass_ID);
 		                		     		        	$("#contentGrName2").val(responseJson.studentID);		                		     		        	
 	 		                		     		        $("#classID2").val(responseJson.classID);
 	 		                		     		    	$("#mark").val(responseJson.mark);
 		                		     		    }
 		                		           	 );
 		                		...
 		                		.append($("<a>")
 		                				...
 		                				.click(function(){
 		                					var retVal = confirm("Do you really want to delete this?");          	
	 		           	                    if( retVal == true ){
	 		           	                    	$.post("act_StudentClass",{action: '4',studentclass_ID : product.studentclass_ID}, 
	   		                						function(responseJson) {  
		   		                						if (responseJson.check == "fail") {
        	...
        	$('#add1').click(function(){
           	 $.post("act_StudentClass",{action: '1',studentID : $('#studentID').val(),classID : $('#classID').val()},
        	...
        	$('#edit1').click(function(){
           	 $.post("act_StudentClass",{action: '2',studentclass_ID : $('#contentGrID2').val(),
           		 studentID : $('#contentGrName2').val(), classID : $('#classID2').val()},
           	 function(responseJson) {          // Execute Ajax GET request on URL of "someservlet" and execute the following function with Ajax response JSON...
           		if (responseJson.check == "fail") {
                    var retVal = confirm("Please enter your full details with your request!\nDo you want to continute?");          	
                    if( retVal == true ){
   ...
   <!-- Form add Option, sử dụng chung với content group -->
        ...
        <div class="md-content">
            ...
            <form id="login_form_contentGr" class="was-validated" action="" method="">
                ...
                <!--  
                        ...
                        -->
                        <div>
                            <label for="text" class="lbl_contentgr">Student ID</label>
                            <select id="studentID" class="custom-select d-block my-3 select_contentgr" required
                            style="width:75%;">
                        ...
                        <div>
                            <label for="text" class="lbl_contentgr">Class ID</label>
    ...
    <div class="md-modal113" id="modal-4options">
        <div class="md-content">
            ...
            <form id="login_form_contentGr2" class="was-validated" action="" method="post">
                <fieldset>
                        <div>
                            <label for="text">StudentClass ID</label>
                        </div>
                        <div>
                            <input type="text" name="studentclass_ID" id="contentGrID2" placeholder="Enter Authority ID" 
                        ...
                        <div>
                            <input type="text" name="studentID" id="contentGrName2" placeholder="Enter Authority ID" 
                              disabled style="width:85%;">
                        </div>
    ...
    <!-- Authority Manegement -->
     ...
     <div class="container tieude" id="content-gr">
         		<h2 class="text-center" style="color: red;font-weight: bold;margin-bottom: 20px !important;">STUDENT'S CLASS MANEGEMENT</h2>
        <p class="text-right">
					...
					<table class="table table-bordered table-hover table-striped paginated" id="productTable">
						<thead>
							<tr class="text-center">
								<th class="text-center">StudentClass ID</th>
